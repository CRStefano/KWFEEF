"""
Life-history analysis (Eq. 11 of the paper): iteroparity/semelparity transition
built ON TOP OF the S-FEEF foraging model.

P0 = 1 - p_dead(pi*) is the baseline per-cycle survival, fixed by the optimal
S-FEEF perceptual policy and the horizon tau. The lineage fitness is
Phi(u) = u^gamma / (1 - P0 (1-u)^k). The optimal strategy u* jumps from interior
(iteroparity) to 1 (semelparity) when gamma exceeds gamma_c = P0 (k=1).
"""
import numpy as np
...
def main():
    with open('lifehistory_results.txt', 'w') as f:
        ...
        f.write("gamma_c = P0 (iteroparity -> semelparity transition)\n\n")
        f.write("Foraging mortality (via tau) -> optimal strategy (gamma=0.85):\n")
    ...
    print("wrote lifehistory_results.txt")
