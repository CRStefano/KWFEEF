"""Test: is the non-monotonicity of the Pareto frontier a robust result
or an artefact of the 'centroid' motor rule? We compare 3 motor rules
with everything else held equal (same state space, same viability, same MI).

Motor rules (action given the perceived food CLASS):
  centroid     : move toward the grid cell nearest the class centroid (CURRENT)
  random       : move toward a RANDOM member of the class (uniform mixture)
  bayes        : action maximising the expected prob. of eating given the posterior over the class
"""
import numpy as np
...
def class_of(partition):
    """map f -> list of food members of its class"""
    cm = {}
...
def action_probs(m, pos, members, rule):
    """return dict {destination_cell: prob} for the given rule."""
    if not members:
    ...
    if rule=='bayes':
        ...
        for a2 in grid_neighbours(pos):
            ...
            tie=np.mean([_manh(a2,CELLS[c]) for c in members])  # tie-break: mean proximity
...
print('rule      | envelope V per I level (0,0.72,0.97,1.37,1.52,1.92,2.32) | dips over_pen I*')
