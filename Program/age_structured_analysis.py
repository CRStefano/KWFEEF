"""
Analysis of the AGE-STRUCTURED model coupled to S-FEEF foraging.
Shows: (1) emergent senescence; (2) how foraging mortality P0
(via horizon tau) regulates lifespan and the rate of senescence.
"""
import numpy as np
...
def main():
    with open('age_structured_results.txt', 'w') as f:
        ...
        f.write("(1) Emergent senescence (P0=0.92, gamma=0.6):\n")
        ...
        f.write("  -> u(a) rises, s(a) falls: senescence with no imposed deterioration.\n\n")

        f.write("(2) Foraging mortality (tau) -> lifespan and regime (gamma=0.6):\n")
        f.write("  tau  p_dead    P0   life_exp   peak_age  regime\n")
        for tau in [2, 3, 4, 5, 7, 10]:
            pd = optimal_policy_pdead(tau); P0 = 1 - pd
            r = optimise_schedule(P0=P0, gamma=0.6, A=15)
            reg = 'semelparous' if is_semelparous(r) else 'iteroparous'
            f.write("  %3d  %.3f   %.3f    %.2f        %2d       %s\n" % (
                tau, pd, P0, life_expectancy(r), age_at_peak_reproduction(r), reg))

        f.write("\n(3) Reproductive returns (gamma) -> iteroparity/semelparity (P0=0.90):\n")
        for g in [0.5, 0.7, 0.9, 1.0, 1.4]:
            r = optimise_schedule(P0=0.90, gamma=g, A=15)
            real = r['l'] * r['m']; conc = real.max() / real.sum()
            reg = 'SEMELPAROUS' if is_semelparous(r) else 'iteroparous'
            f.write("  gamma=%.1f: reprod concentration=%3.0f%%  life=%.2f  %s\n" % (
                g, 100 * conc, life_expectancy(r), reg))
    print("wrote age_structured_results.txt")
