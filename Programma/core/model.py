"""SemanticForagingModel - framework S-FEEF (revisione C, ambiente 2-D).

Ambiente bidimensionale: 5 celle di cibo disposte ai 4 angoli e al centro di una
griglia 3x3 (9 posizioni dell'agente). Le policy percettive sono le B_6 = 203
partizioni di Y = {5 celle} U {assente}. Sotto una partizione che fonde celle
spazialmente opposte il centroide percepito cade su una cella SBAGLIATA: la
percezione grossolana diventa attivamente FUORVIANTE, e la viability V(I) e' non
monotona (l'informazione in eccesso puo' ridurre la viability).

Resta tutto il resto della revisione B: viability Eq. (2), accoppiamento causale
via logical_target, substrato termodinamico di Metropolis che soddisfa il Local
Detailed Balance a precisione macchina.
"""
from __future__ import annotations
import numpy as np
from . import information as info_mod

# 5 celle di cibo: 4 angoli + centro di una griglia 3x3
CELLS = {0: (0, 0), 1: (0, 2), 2: (1, 1), 3: (2, 0), 4: (2, 2)}
GRID = [(x, y) for x in range(3) for y in range(3)]   # 9 posizioni agente
N_FOOD = 5
NO_FOOD = N_FOOD          # indice 5 = cibo assente
N_Y = N_FOOD + 1          # |Y| = 6


def set_partitions(n: int):
    """Tutte le partizioni di {0,...,n-1} (numero di Bell B_n; B_6 = 203)."""
    if n <= 0:
        yield []
        return
...
class SemanticForagingModel:
    ...
    def _assign_free_energies(self):
        ...
        for idx, (pos, f, v) in enumerate(self.states):
            ...
            if f != NO_FOOD:
                F += gamma_food
            self.F[idx] = F
        # distribuzione di Boltzmann (equilibrio termico) pi_k ~ exp(-F_k):
        # l'equilibrio e' dominato dagli stati morti (F basso). La viability e'
        # la distanza da questo equilibrio (vedi _viability).
    ...
    def equilibrium_reference_matrix(self):
        """Catena di Metropolis su F: substrato termodinamico (LDB esatto)."""
        n = self.num_states
        M = np.zeros((n, n))
    ...
    def _class_representative(self, cls):
        """Cella di griglia piu' vicina al centroide spaziale della classe.
        NESSUNA protezione: una classe che fonde celle opposte produce un
        centroide su una cella sbagliata (percezione fuorviante)."""
    ...
    def _step(self, pos, tgt):
        """Un passo di Manhattan verso (mode pos) o lontano da (neg) il target.
        mode neutral o target assente -> resta (random walk gestito a monte)."""
        if tgt is None:
            return pos
        if self.mode == 'neg':
            best = pos
    ...
    def _viability(self, p_tau):
        # Viability = energia libera di non-equilibrio = D_KL(p_tau || pi_eq) in
        # bit: la distanza dall'equilibrio termico (Boltzmann), cioe' la nozione
        # di viability di Kolchinsky-Wolpert, DERIVATA dalle energie libere F
        # invece che postulata. Nessun parametro di penalita' lambda separato:
        # la profondita' dello stato morto e' gia' dentro pi_eq.
    ...
    def compute_pareto_front(self, initp, alpha=9.5, n_points=None):
        ...
        # scatter completo conservato per la figura della non-monotonicita'
