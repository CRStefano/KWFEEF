        ...
        try:
            ...
            if math.isfinite(sst) and sst > 0:
                fig.add_vline(x=sst, line_width=1, line_dash="solid", line_color="#10b981",
                              annotation_text="SST", annotation_position="top right")
            if math.isfinite(i_star):
                fig.add_vline(x=i_star, line_width=1, line_dash="solid", line_color="#f59e0b",
                              annotation_text="Min S-FEEF Policy",
                              annotation_position="bottom right")

            fig.update_layout(
                plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
                margin=dict(l=20, r=20, t=40, b=20),
                title=dict(text="S-FEEF Policy Selection: Intrinsic Cost vs Extrinsic Viability",
                           font=dict(color='#f8fafc', size=14)),
                hovermode="x unified",
                xaxis=dict(title="I(X₀; Y₀|π) | Computational Cost (Bits)",
                           gridcolor='rgba(255,255,255,0.1)', color='#94a3b8'),
                yaxis=dict(color='#94a3b8', gridcolor='rgba(255,255,255,0.1)'),
                yaxis2=dict(color='#94a3b8', showgrid=False),
