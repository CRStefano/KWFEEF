"""Sweep di parametri sui modelli S-FEEF + life-history: cosa e' robusto, cosa
emerge. Genera sweep_results.txt."""
import numpy as np
...
def analyse(mode='pos', tau=5, fdr=0.1, er=1, de=100.0, fe=10.0, alpha=9.5):
    ...
    # non-monotonicita': numero di cali nell'envelope + penalita' sopra-soglia
    dips = int(np.sum(np.diff(vs) < -1e-6))
    vmax = vs.max(); vfull = vs[-1]
    over_pen = float(vmax - vfull)            # >0 => info oltre il picco danneggia
    # misleading gap: a parita' di I, spread V_best - V_worst (sul cloud)
...
out = []
out.append("="*78)
out.append("A) SWEEP FORAGING S-FEEF (modello 2-D). baseline: tau5 fdr0.1 er1 de100 fe10")
out.append("="*78)
base = analyse()
...
out.append("")
out.append("  -- eat_radius (affordance corporea) --")
for er in [0,1,2]:
...
out.append("  -- food_disappear_rate (volatilita') --")
...
out.append("  -- tau (orizzonte) --")
for tau in [3,5,7,10]:
    r=analyse(tau=tau); out.append(f"   tau={tau}: SST={r['SST']:.3f} I*={r['Istar']:.3f} a_c={r['ac']:.2f} dips={r['dips']}")

out.append("")
out.append("="*78)
out.append("B) LIFE-HISTORY: universalita' di gamma_c = P0")
out.append("="*78)
for P0 in [0.3,0.5,0.7,0.9]:
    gc=critical_gamma(P0)
    out.append(f"   P0={P0:.2f} -> gamma_c={gc:.3f} (atteso {P0:.3f}); match={abs(gc-P0)<0.01}")

out.append("")
out.append("="*78)
out.append("C) BET-HEDGING: soglia sigma* (passaggio a iteroparita') vs P0")
out.append("="*78)
for P0 in [0.92,0.80,0.70]:
    ...
    for sig in np.linspace(0,1.6,33):
        ...
        prev=u
    out.append(f"   P0={P0:.2f}: sigma a cui u* scende sotto 0.5 ~ {sstar}")
